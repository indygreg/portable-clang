...
/*! LLVM option/argument parsing functionality.

This functionality should be generic to most (all?) LLVM programs which
...
#[cfg(target_family = "unix")]
fn argument_matches_prefix<'a>(arg: &ProgramOption, s: &'a OsStr) -> Option<&'a OsStr> {
    ...
    for prefix in &arg.prefixes {
        ...
        if s_bytes.starts_with(search.as_bytes()) {
            return Some(OsStr::from_bytes(&s_bytes[search.as_bytes().len()..]));
        }
    }
...
impl ProgramOption {
    ...
    pub fn matches_prefix<'a>(&self, s: &'a OsStr) -> Option<&'a OsStr> {
...
pub struct CommandOptions {
    pub(crate) options: Vec<ProgramOption>,
}
...
impl CommandOptions {
    ...
    pub fn options_by_flag(&self) -> HashMap<&str, Vec<&ProgramOption>> {
        ...
        for o in &self.options {
            for flag in &o.flags {
                h.entry(flag.as_str()).or_insert(vec![]).push(o);
            }
        }
    ...
    pub fn options_by_group(&self) -> HashMap<&str, Vec<&ProgramOption>> {
        ...
        for o in &self.options {
            if let Some(group) = &o.group {
                h.entry(group.as_str()).or_insert(vec![]).push(o);
            }
        }
...
pub struct ParsedArguments {
    pub(crate) parsed: Vec<ParsedArgument>,
}
